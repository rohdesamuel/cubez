...
struct Market {
  std::unordered_map<Resource, std::vector<BuyOrder>> buy_orders;
  std::unordered_map<Resource, std::vector<SellOrder>> sell_orders;

  BuyContract* Buy(BuyOrder order);
  BuyContract* Buy(Resource resource, uint32_t amount, uint32_t max_price);

  SellContract* Sell(SellOrder order);
  SellContract* Sell(Resource resource, uint32_t amount, uint32_t min_price);
};
...
namespace std
{
  template<> struct hash<LocationRef> {
    std::size_t operator()(LocationRef const& location) const noexcept {
      return std::hash<uint64_t>{}((uint64_t)(location.planet_id) << 32 | location.tile_id);
    }
  };
}
...
struct Corporation {
  ...
  std::unordered_map<Resource, ResourceBin> resources;
  std::unordered_map<LocationRef, uint32_t> warehouse_locations;

  std::unordered_map<Resource, std::vector<BuyContract>> buy_contracts;
  std::unordered_map<Resource, std::vector<BuyContract>> sell_contracts;

  std::vector<LocationRef> owned_land;
  std::vector<LocationRef> undeveloped_land;

  static void build_residences(Corporation& corp) {
