...
Overlay::Overlay(ultralight::Ref<ultralight::Renderer> renderer,
  ...
  int width, int height, int x, int y, float scale, bool transparent) :
  view_(renderer->CreateView(width, height, transparent)), width_(width), height_(height),
  x_(x), y_(y), needs_update_(true), driver_(driver), transparent_(transparent) {
}
...
void Overlay::UpdateGeometry() {
  ...
  v.color[1] = 255;
  v.color[2] = 255;
  v.color[3] = (char)(transparent_ ? 0 : 255);
