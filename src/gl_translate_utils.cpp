...
GLenum TranslateQbBlendFactorToOpenGl(qbBlendFactor factor) {
  switch (factor) {
    ...
    case QB_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA: return GL_ONE_MINUS_SRC_ALPHA;
...
GLenum TranslateQbBufferAccessToOpenGl(qbBufferAccess access) {
  switch (access) {
    case QB_BUFFER_ACCESS_READ_ONLY: return GL_READ_ONLY;
    case QB_BUFFER_ACCESS_WRITE_ONLY: return GL_WRITE_ONLY;
    case QB_BUFFER_ACCESS_READ_WRITE: return GL_READ_WRITE;
  }
  assert(false && "Invalid qbBufferAccess");
  return 0;
}